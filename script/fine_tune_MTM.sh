...
LOAD_CHECKPOINT_FOLDER="$SRC_FOLDER/checkpoints_bak/P1/EXP2"


cd $SRC_FOLDER
mkdir -p $CHECKPOINT_FOLDER
mkdir -p $SRC_FOLDER/pickle_MTL/Fine_tune/${1}_$3
...
python3 -u $SRC_FOLDER/train_p1.py \
  ...
  --train_file \
  $SRC_FOLDER/corpus/$1/train_${3}.tsv \
  --dev_file \
  $SRC_FOLDER/corpus/$1/devel.tsv \
  --test_file \
  $SRC_FOLDER/corpus/$1/test.tsv \
  ...
  --word_hidden 300 \
  --drop_out 0.5 \
  --pickle $SRC_FOLDER/pickle_MTL/Fine_tune/${1}_$3 \
  --idea P10 \
  --pred_method M \
  --mask_value 0 \
  --load_checkpoint_crf 0 \
