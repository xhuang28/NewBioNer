...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_loader', help='the file of crf2train loader')
    parser.add_argument('--dev_loader', help='the file of crf2dev loader')
    parser.add_argument('--dev_loader2', help='the file of dev dataset loader')
    parser.add_argument('--test_loader', help='the file of test dataset loader')
    ...
    #crf2train_dataloader = pickle.load(open(args.train_loader, 'rb'))
    ...
    with open(args.train_loader, 'rb') as f:
        while True:
            try:
                l = pickle.load(f)
                # make up a mistake in making predictions
                l[8] = autograd.Variable(l[8])
                l[5] = autograd.Variable(l[5])
                crf2train_dataloader.append(l)
            except:
                break
    ...
    crf2dev_dataloader = pickle.load(open(args.dev_loader, 'rb'))
    ...
    # for item in itertools.chain.from_iterable(dev_dataset_loader[0]):
        ...
        # print(item[8][0])
        # break
    dev_dataset_loader = pickle.load(open(args.dev_loader2, 'rb'))
    ...
    ner_model = LM_LSTM_CRF(len(tag2idx), len(chr2idx), 
        ...
        in_doc_words=in_doc_words, highway_layers = args.highway_layers)
    ...
    test_dataset_loader = pickle.load(open(args.test_loader, 'rb'))
