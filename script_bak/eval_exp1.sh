...
python3 -u $SRC_FOLDER/eval.py \
  --load_arg $LOAD_CHECKPOINT/$PHASE/EXP1/$EXEC_NAME/$CHECKPOINT_NAME.json \
  --load_check_point $LOAD_CHECKPOINT/$PHASE/EXP1/$EXEC_NAME/$CHECKPOINT_NAME.model \
  --if_pred \
  --pred_file \
  ...
  --pickle $5 \
  --pred_method $2 \
  | tee $SRC_FOLDER/logs/$PHASE/eval_exp1_$2._$EXEC_NAME.log
