...
# 1: Domain name; 2: # epochs
EXEC_NAME="CONLL_STM_${1}_C100_W300_EP${2}"
...
mkdir -p $SRC_FOLDER/pickle_conll_STM2_${1}
...
python3 -u $SRC_FOLDER/train_p1.py \
  ...
  --batch_size 10 \
  --least_iters $2 --epoch $2 --patience 30 --stop_on_single \
  --lr 0.01 \
  --gpu 0 \
  --char_hidden 100 \
  --word_hidden 300 \
  --drop_out 0.5 \
  --pickle $SRC_FOLDER/pickle_conll_STM2_${1} \
  --combine \
  --idea P10 \
  --pred_method M \
  --mask_value 0 \
