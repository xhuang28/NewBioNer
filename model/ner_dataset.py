...
class CRFDataset_WC(Dataset):
    ...
    """
    def __init__(self, forw_tensor, forw_index, back_tensor, back_index, word_tensor, label_tensor, mask_tensor, len_tensor, missing_tagspace, reorder):
        #print(forw_tensor.size(), label_tensor.size())
        ...
        assert forw_tensor.size(0) == missing_tagspace.size(0)
        assert forw_tensor.size(0) == len(reorder)
        ...
        self.missing_tagspace = missing_tagspace
        self.reorder = reorder

    def __getitem__(self, index):
        return self.forw_tensor[index], self.forw_index[index], self.back_tensor[index], self.back_index[index], self.word_tensor[index], self.label_tensor[index], self.mask_tensor[index], self.len_tensor[index], self.missing_tagspace[index], self.reorder[index]
