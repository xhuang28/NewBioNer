...
class LM_LSTM_CRF(nn.Module):
    ...
    args: 
        ...
        word_embedding_dim: size of word embedding
    ...
    def __init__(self, tagset_size, char_size, char_dim, char_hidden_dim, char_rnn_layers, word_embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, num_crf, large_CRF=True, if_highway = False, in_doc_words = 2, highway_layers = 1):
        ...
        self.word_dim = word_embedding_dim
        ...
        self.word_lstm = nn.LSTM(word_embedding_dim + char_hidden_dim * 2, word_hidden_dim // 2, num_layers=word_rnn_layers, bidirectional=True, dropout=dropout_ratio)
    ...
    def rand_init(self, init_char_embedding=True, init_word_embedding=False, init_pos_embedding=True, init_chunk_embedding=True):
        """
        ...
        if init_char_embedding:
            print("inin char embed")
            utils.init_embedding(self.char_embeds.weight)
        if init_word_embedding:
            print("inin word embed")
    ...
    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq, crf_no, corpus_mask_v, hidden=None):
        '''
        args:
        ...
        #convert to crf
        #combine
        crf_input = d_lstm_out

        crf_out = self.crflist[crf_no](crf_input)
