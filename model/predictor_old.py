...
class predict:
    ...
    def generate_combined_training(self, ner_model, documents, fout, dataset_no, dataset_id, gold, strategy="best"):
        """ used for generate combined training corpus
        args:
            ner_model: sequence labeling model
            documents = feature (list): list of words list
            scope (list):
            fout: output file
        """
        ...
        for d_ind in tqdm(range(0, d_len), mininterval=1,
            ...
            for ind in range(0, f_len, self.batch_size):
                eind = min(f_len, ind + self.batch_size)
                seperate_predictions = []  # [[path_score, path]]
                seperate_predictions.append((float('inf'), gold))
                for i in range(dataset_no):  # iterate on five CRF
                    if i == dataset_id:  # do not make prediction on golden data
                        continue 
            ...
            for ind in range(0, f_len, self.batch_size):
                ...
                for i in range(dataset_no):  # iterate on five CRF
                    ...
                    for ind2 in range(ind, eind):
                        ...
                        l_idx = labels[ind2 - ind][0: len(f)]
                        l = [self.r_l_map[label] for label in l_idx]
                        if scope != ['ALL']: # remove tags that are not in scope
                            for i in range(len(l)):
                                if l[i][2:].upper() not in scope:
                                    l[i] = 'O'
                        seperate_predictions.append((i, l))
                if strategy == "vote": # todo
                    combine = self.combine_prediction_vote(seperate_predictions)
                else:  # best path
                    combine = self.combine_prediction(seperate_predictions)
