...
checkpoint = '/auto/nlg-05/huan183/NewBioNer/checkpoints/P1/EXP2/MTL_C100_W300/N2N_BC4CHEMD-IOBES_LAST_0.9706_0.9616_0.9797_425'
...
for data_name, data_file in [['BioNLP11ID', '/auto/nlg-05/huan183/NewBioNer/corpus/eval/BioNLP11ID-IOBES/test.tsv'], \
    ...
    # for i in range(len(preds[0])):
        # curr_preds = [[args.idx2tag[rr] for rr in r[i].numpy().tolist() if rr!=args.tag2idx['<pad>']] for r in preds]
        # curr_labels = [args.idx2tag[rr] for rr in labels[0][i].numpy().tolist() if not rr in [args.tag2idx['<pad>'], args.tag2idx['<start>']]]
        # curr_vote = []
        # for j in range(len(curr_preds[0])):
            # curr_curr_pred = [r[j] for r in curr_preds]
            # curr_vote.append(max(set(curr_curr_pred), key=curr_curr_pred.count))
        
        # evaluator.eval_sent(curr_vote, curr_labels)
