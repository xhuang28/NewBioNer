...
EXEC_NAME="CN21_C${1}_W${2}_DP${3}_MV${4}_PR${7}"
...
python3 $SRC_FOLDER/ctrain.py \
  ...
  --dispatch N21 --corpus_mask_value $4 \
  --least_iters 100 --epoch 500 --patience 30 --stop_on_single \
  --lr 0.01 \
  --gpu $7 \
  ...
  --load_opt \
  --combine \
  --pickle $SRC_FOLDER/pickle/comb/$4 \
  | tee $SRC_FOLDER/log/nll/$EXEC_NAME.log
