...
if __name__ == "__main__":
    ...
    with open(args.data_loader + '/' + args.idea + '_' + suffix + '.p', 'rb') as f:
        while True:
            try:
                f_f, f_p, b_f, b_p, w_f, silver, mask_v, len_v, corpus_mask_v, reorder = pickle.load(f)
                crf2train_dataloader.append([f_f, f_p, b_f, b_p, w_f, silver, mask_v, len_v, corpus_mask_v, reorder])
            except:
                break
    
    crf2train_dataloader = [crf2train_dataloader]
