...
class CRF_L(nn.Module):
    ...
    def forward(self, feats):
        ...
        """
        return self.sigmoid(self.hidden2tag(feats).view(-1, self.tagset_size, self.tagset_size))
...
class CRFLoss_vb(nn.Module):
    ...
    def restricted_forward_algo_v1(self, scores, target, mask, corpus_mask):
        ...
        for idx, cur_values in seq_iter:
            ...
            cur_partition = cur_partition * autograd.Variable(torch.FloatTensor(partition_mask)).cuda()
            cur_values = cur_values * autograd.Variable(torch.FloatTensor(values_mask)).cuda()
    ...
    def restricted_forward_algo_v2(self, scores, target, mask, corpus_mask):
        # Restricted Forward Algorithm v1
    ...
    def forward(self, scores, target, mask, corpus_mask, idea=None):
        """
        args:
        ...
        # Li's masking approach
            ...
            numerator = restricted_forward_algo_v1(scores, target, mask, corpus_mask)
            denominator = forward_algo(scores, target, mask, corpus_mask)
        elif idea == "P12":
            numerator = calc_energy_gold_ts(scores, target, mask, corpus_mask)
            denominator = restricted_forward_algo_v2(scores, target, mask, corpus_mask)
        else:
